...
class GANTrainer(object):
    ...
    def load_network_stageI(self):
        ...
        if cfg.NET_G != '':
            state_dict = \
                paddle.load(cfg.NET_G,
                           map_location=lambda storage, loc: storage)
            netG.set_state_dict(state_dict)
            print('Load from: ', cfg.NET_G)
        if cfg.NET_D != '':
            state_dict = \
                paddle.load(cfg.NET_D,
                           map_location=lambda storage, loc: storage)
            netD.set_state_dict(state_dict)
    ...
    def load_network_stageII(self):
        ...
        if cfg.NET_G != '':
            ...
            state_dict = \
                paddle.load(cfg.STAGE1_G,
                           map_location=lambda storage, loc: storage)
            netG.STAGE1_G.set_state_dict(state_dict)
    ...
    def train(self, data_loader, stage=1):
        ...
        optimizerD = \
            optim.Adam(parameters=netD.parameters(),
        ...
        for p in netG.parameters():
            if not p.stop_gradient:
                netG_para.append(p)
        optimizerG = optim.Adam(parameters=netG_para,
        ...
        for epoch in range(self.max_epoch):
            ...
            if epoch % lr_decay_step == 0 and epoch > 0:
                generator_lr *= 0.5
                optimizerG.set_lr(generator_lr)
                discriminator_lr *= 0.5
                optimizerD.set_lr(discriminator_lr)
            ...
            print('''[%d/%d][%d/%d] Loss_D: %.4f Loss_G: %.4f Loss_KL: %.4f
                  ...
                  % (epoch, self.max_epoch, i, len(data_loader),
                     errD.detach()[0], errG.detach()[0], kl_loss.detach()[0],
    ...
    def sample(self, datapath, stage=1):
        ...
        while count < num_embeddings:
            ...
            txt_embedding = paddle.empty([embeddings_batch]).astype('float32')
            noise = paddle.empty([batch_size, nz]).astype('float32')
            # if cfg.CUDA:
            #     txt_embedding = txt_embedding.cuda()
